...
The caller uses `process_output` to retrieve output and check completion status, `process_stop` to terminate the process, and `process_list` to list all currently active background processes.
...
| `running_time` | string | Human-readable time elapsed since the command started (e.g., "1h 21m 39s"). |
